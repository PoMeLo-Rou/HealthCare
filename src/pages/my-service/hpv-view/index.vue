<template>
	<view class="vaccine-order" v-for="(item,index) in order" :key="index">
		<view class="order-tit">{{item.hpv_name}}</view>
		...
		<view class="order-message">
			<text>疫苗套餐: </text>
			<text class="message">{{item.ino_time}} {{item.combo}} </text>
		</view>
		<view class="order-message">
			<text>订单编号: </text>
...
<script setup lang="ts">
	...
	import {Hpvuserorder} from '@/public/decl-type'
	import {onShow} from '@dcloudio/uni-app'
	import Point from '@/com-components/point.vue'
	// 接收新冠疫苗预约订单
	let order = ref<Hpvuserorder[]>([])
	// 定义组件是否显示
	let show = ref(false)
	onShow(async() => {
